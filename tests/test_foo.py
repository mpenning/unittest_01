...
def test_foo_spam_get_words_01():
    ...
    use a context manager to patch 'random.choices()' inside
    ...
    'from my_module.things import WordSpam'.
    """
    # patch() 'my_module.things.random.choices()' in a context manager
    with patch.object(
            my_module.things.random,
            "choices",
            return_value=["fish", "dish"]
        ):
...
def test_foo_spam_get_words_02():
    ...
    """
    ## Yes
    with patch("my_module.things.random.choices") as mock_random_choices:
        # Yes  ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ <- A) Do THIS!!!
...
def test_foo_spam_get_words_03(mock_choices):
    ...
    """
    The VERY GOOD option... patch 'random.choices()' inside 'my_module/things.py'
...
def test_wordspam_05_antipattern():
    """
    Always avoid patching the python stdlib directly; in this case, do NOT patch
    stdlib 'random.choices()'.  Patch it locally in your own module (for the 
    best option, refer to 'test_wordspam_01()', above.)
